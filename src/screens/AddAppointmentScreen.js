...
export default function AddAppointmentScreen({ navigation }) {
  ...
  const validateForm = async () => {
    ...
    // Verificar duplicados solo si no hay errores en fecha y vehículo
    if (!newErrors.date && !newErrors.vehicleModel && formData.date && formData.vehicleModel) {
      try {
        ...
        newErrors.duplicate = 'Error al verificar citas duplicadas';
      }
    }
  ...
  const handleSave = async () => {
    if (await validateForm()) {
      try {
        await AppointmentService.saveAppointment(formData);
        Alert.alert(
          'Éxito',
